...
class DigitalResourceController extends Controller
{
    ...
    public function update(Request $request, Record $record)
    {
        ...
        try {
            // Handle file upload if present
            $coverImagePath = $record->digitalResource->resource_cover_thumbnail;
            ...
            // Redirect with success message
            return redirect()->route('digital.show', $record)
                ->with('success', 'Digital Resource "' . $record->title . '" created successfully!');


        } catch (\Exception $e) {
            $errorMessage = app()->environment('local') // will only show this in local
                ? 'Failed to update digital resource: ' . $e->getMessage()
                : 'Failed to update digital resource. Please try again.';
